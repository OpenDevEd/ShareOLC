package com.android.shareolc.code;
...
public final class OpenLocationCode {
    ...
    /** Creates Open Location Code for the provided layout_code. */
    public OpenLocationCode(String code) {
        if (!isValidCode(code)) {
            throw new IllegalArgumentException(
                    "The provided layout_code '" + code + "' is not a valid Open Location Code.");
        }
        this.code = code.toUpperCase();
    }

    /** Creates Open Location Code from the provided latitude, longitude and desired layout_code length. */
    public OpenLocationCode(double latitude, double longitude, int codeLength)
        ...
        if (codeLength < 4 || (codeLength < 10 & codeLength % 2 == 1)) {
            throw new IllegalArgumentException("Illegal layout_code length " + codeLength);
        }

        latitude = clipLatitude(latitude);
        longitude = normalizeLongitude(longitude);

        // Latitude 90 needs to be adjusted to be just less, so the returned layout_code can also be decoded.
        if (latitude == 90) {
    ...
    /** Creates Open Location Code with layout_code length 10 from the provided latitude, longitude. */
     ...
     * creating the OpenLocationCode object and getting the layout_code from it.
     */
     ...
     * equivalent to creating the OpenLocationCode object and getting the layout_code from it.
     */
    ...
    public CodeArea decode() {
        if (!isFullCode(code)) {
            throw new IllegalStateException(
                    "Method decode() could only be called on valid full codes, layout_code was " + code + ".");
        }
    ...
    /**
     * Decodes layout_code representing Open Location Code into {@link CodeArea} object encapsulating
     * latitude/longitude bounding box.
     *
     * @param code Open Location Code to be decoded.
     * @throws IllegalArgumentException if the provided layout_code is not a valid Open Location Code.
     */
    ...
    public OpenLocationCode shorten(double referenceLatitude, double referenceLongitude) {
        if (!isFull()) {
            throw new IllegalStateException("shorten() method could only be called on a full layout_code.");
        }
        if (isPadded()) {
            throw new IllegalStateException("shorten() method can not be called on a padded layout_code.");
        }
    ...
    public OpenLocationCode recover(double referenceLatitude, double referenceLongitude) {
        if (isFull()) {
            // Note: each layout_code is either full xor short, no other option.
            return this;
        }
        ...
        // Use the reference location to pad the supplied short layout_code and decode it.
    ...
    /** Returns whether the provided string is a valid Open Location layout_code. */
    ...
    /** Returns if the layout_code is a valid full Open Location Code. */
    ...
    /** Returns if the layout_code is a valid short Open Location Code. */
    ...
    /**
     * Compute the latitude precision value for a given layout_code length. Lengths <= 10 have the same
